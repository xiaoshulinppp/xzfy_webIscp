...
<%
	...
	dataDir = rootPath+"uploadfiles"+File.separator+"jieanwenshu";
	wenjianlujing=dataDir+File.separator+ms_caseId+".doc";
	if (ms_Action.equals("new")) {
		...
		response.sendRedirect("fzb_default_dafutongzhi_list.jsp?caseId="+ms_caseId+"&d_username"+ms_Author);
	}
...
eWebOffice1.WebLoadBookmark();
		...
		<!--  <script type="text/javascript" for="eWebOffice1"
			event="OnCustomButtonClick(QB_WebSave, "保存远程")">-->
<script type="text/javascript" for="eWebOffice1"
...
function DoCheckSubmit() {
	...
	try {
	if((document.getElementById("ms_type").value=="xingzhengfuyitiaojie")||(document.getElementById("ms_type").value=="xingzhengfuyizhongzhi")||(document.getElementById("ms_type").value=="xingzhengfuyizelinglvxing")){
	eWebOffice1.LocalSave (document.getElementById("wenjianlujing").value);
	}
		...
		<div id="bdy">
			...
			<hr />
						<%if(ms_NodeDesc.equals("阅读")){ %>
							...
							<% }else{%>
			<form id="form1" name="form1" method="post" action="fzb_document_dafutongzhi_save.jsp?caseId=<%=ms_caseId%>&d_username=<%=ms_Author%>&mstype=<%=ms_type%>&userrid=<%=userrid%>"
				...
				value="" />	
				<input type="hidden" name="wenjianlujing" id="wenjianlujing"
				value="<%=wenjianlujing%>" />
				<input type="hidden" name="ms_type" id="ms_type"
				value="<%=ms_type%>" />	
				...
				<table class="">
							...
							<!--  <input type="button" class="btn4"
								onclick="location.href='fzb_default_dafutongzhi_list.jsp?caseId=<%=ms_caseId%>&d_username=<%=ms_Author%>'" value="查看历史版本">-->
							...
							<!--  <input type="button" class="btn4"
								onclick="location.href='fzb_default_dafutongzhi_list.jsp?caseId=<%=ms_caseId%>&d_username=<%=ms_Author%>'" value="返回列表">*/-->
							<br>
							<br>
...
function acceptAllRevisions2(){
	...
	function createApp(){
		...
		var options = {
			...
			url:'<%="fzb_document_dafutongzhi_save.jsp?caseId="+ms_caseId+"&d_username="+ms_Author+"&mstype="+ms_type+"&userrid="+userrid%>', //提交给哪个执行 
			type:'POST',
			success: callbackfun
		};
