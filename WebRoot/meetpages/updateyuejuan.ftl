...
<div class="wal">
	...
	<input type="hidden" name="meet.Meet_type" value="阅卷"/>
	...
	<input type="hidden" name="meet.Law_id" value="${xzfyInfo.id}"/>
		<table width="98%" cellpadding="0" cellspacing="0" border="0">
			...
			<tr >
    			<th width="20%">接待日期</th>
    			<td width="30%">
				<input type="text" name="meet.Face_date" id="Face_date" value="${todate?default('')[0..9]}"  onClick="WdatePicker()" />
      			</td>
				<th width="20%">接待地点</th>
				<td width="30%">
			...
			<tr >
				<th  >被接待人数</th>
				<td >
					<input type="text"  id="rencount" onchange="checkRen();countNum('rencount')" name="meet.Face_allnumber" value="0" />
				</td>
				<th  >&nbsp;</th>
				<td >&nbsp;
 ...
 <tr >
	 <th  >阅卷笔录内容：<br>(鼠标双击添加对话)</th>
	 <td  colspan=3>
	  <textarea name="meet.Face_content"  class="textarea" id="answerValue" type="text"  ondblclick="getValue(this)">
接待人询问：今天请在此查阅“${xzfyInfo.tjzfzh?default('')}”行政复议案卷。
被接待人回答：我已经查阅该案卷。
接待人询问：申请人阅卷后，如有意见，可以及时以书面方式提交我们。
被接待人回答：好的。
</textarea>
</td>
	 </tr>
   ...
   <script>
   		if(document.getElementById("rencount").value>1){
   			//alert(document.getElementById("answerValue").value)
   			document.getElementById("answerValue").value="接待人询问：今天请在此查阅“${xzfyInfo.tjzfzh?default('')}”行政复议案卷。\n被接待人回答：我们已经查阅该案卷。\n接待人询问：请问被接待人查阅后有何意见需要陈述？\n被接待人回答：我们将通过提交书面材料表明意见。"
   		}
...
<script type="text/javascript">
	...
	function callbackfun2(){
		var url ="../fzb_default_dafutongzhi.jsp?&d_filetype=doc&d_username=${Session['_USER_LOGIN_'].id}&caseId=${mainidd}&mstype=yuejuanbilu";
