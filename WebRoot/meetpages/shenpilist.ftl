...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		...
		<input type="hidden" name="xzfy.id" value="${xzfyId}">
		...
		<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" class="tabout">
			...
			<tr class="trColor">
				...
				<td class="tdlistCenter">
				<#if data.App_type?has_content>${data.App_type}</#if></td>
				<td class="tdlistCenter">
				<#if data.Defendant_real_name?has_content>${data.Defendant_real_name}</#if></td>
				<td class="tdlistCenter">
				<#if data.User1_name?has_content>${data.User1_name}</#if></td>
				<td class="tdlistCenter">
				<#if data.User2_name?has_content>${data.User2_name}</#if></td>
				<td class="tdlistCenter">
				...
				<td class="tdlistCenter">
				<a href="javascript:updateApp('${data.id}')">审批</a>&nbsp;
				</td>
			</tr>
	...
	<script>
		...
		function updateApp(id，status){
				...
				<#if status = 2 || status=3>  
					document.appForm.action="updateshenpi1.action";
				</#if>
