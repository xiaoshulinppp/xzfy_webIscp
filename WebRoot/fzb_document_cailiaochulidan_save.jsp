...
<%
	...
	if (rs.next()) {
		s_sql = "update eWebOffice_T_Document set D_RecordID=?,D_Subject=?,D_Author=?,D_FileDate=?,D_FileType=?,CASEID=?,D_TEMPLATEID=?,D_TYPE=?,D_ISQINGGAO=?,D_SHENPIDANTYPE=?,D_ISZUIZHONG=? where D_RecordID='"
				+ mRecordID + "'";
	} else {
		s_sql = "insert into eWebOffice_T_Document (D_RecordID,D_Subject,D_Author,D_FileDate,D_FileType,CASEID,D_TEMPLATEID,D_TYPE,D_ISQINGGAO,D_SHENPIDANTYPE,D_ISZUIZHONG) values (?,?,?,?,?,?,?,?,?,?,?)";
	}
	...
	stmt.setString(11, ms_iszuizhong);
