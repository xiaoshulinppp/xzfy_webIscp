...
function AddOption(s_Id, s_Value, s_Text) {
	...
	<body onload="ChangeFileType()">
		<div id="bdy">
			...
			<div id="nav">
				您当前位置 &gt;&gt;
				<a href="fzb_default_shenpidan.jsp?caseId=<%=ms_cassid%>&d_username=<%=ms_userrName%>&d_checkstatus=<%=ms_checkstatus%>&finish_type=<%= ms_finishtype%>></a>">首页</a>
			</div>
			<hr />
			...
			<table class="lst">
				...
				<tr>
					<td>
						<form name="form1" action="fzb_document_shenpidan_edit.jsp?action=new"
							method="post">
							用户名：
...
<%tempUrl = request.getContextPath();
			...
			tempUrl += "fzb_document_shenpidan_edit.jsp?action=new&d_filetype=doc&d_templateid="
					+ d_templateid
					+ "&d_username="
					+ ms_userrName
					+ "&caseId="
			...
			if(con != null){con.close();}
				...
				<%
					...
					try {
						...
						if (length == 0)
						{
							s_alert = "<script>window.location='fzb_document_shenpidan_edit.jsp?action=new&d_filetype=doc&d_templateid="+d_templateid+"&caseId="+ms_cassid+"&d_username="+ms_userrName+"&d_checkstatus="+ms_checkstatus+"&finish_type="+ms_finishtype+"&userrid="+userrid+"'</script>";
							closeAll(con, sttmt, rs);
							response.getWriter().write(s_alert);
						}
						while (rs.next()) {
							if(ms_actiontype.equals("yuedu")){
								s_alert = "<script>window.location='fzb_document_shenpidan_edit.jsp?action=read&caseId="+ms_cassid+"&d_username="+ms_userrName+"&d_recordid="+rs.getString("D_RecordID")+"&userrid="+userrid+"&d_templateid="+d_templateid+"&d_checkstatus="+ms_checkstatus+"'</script>";
								closeAll(con, sttmt, rs);
							response.getWriter().write(s_alert);
							}
							if(rs.getString("D_isqinggao").equals("未清稿")||rs.getString("D_isqinggao").equals("已清稿")){
								s_alert = "<script>window.location='fzb_document_shenpidan_edit.jsp?action=yesrevision&caseId="+ms_cassid+"&d_username="+ms_userrName+"&d_recordid="+rs.getString("D_RecordID")+"&userrid="+userrid+"&d_templateid="+d_templateid+"&d_checkstatus="+ms_checkstatus+"'</script>";
								closeAll(con, sttmt, rs);
							response.getWriter().write(s_alert);
							}
							...
							if (rs.getString("D_FileType").toLowerCase().equals("doc")
									|| rs.getString("D_FileType").toLowerCase().equals(
											"wps")) {
								s_Link = "<a href='fzb_document_shenpidan_edit.jsp?action=norevision&d_templateid="
										+ d_templateid
										+ "&d_username="
										+ ms_userrName
										+ "&d_recordid="
										...
										+ ms_cassid
										+ "'>修改无痕迹</a>"
										+ "→<a href='fzb_document_shenpidan_edit.jsp?action=yesrevision&d_templateid="
										+ d_templateid
										+ "&d_username="
										+ ms_userrName
										+ "&d_recordid="
										...
										+ ms_cassid
										+ "'>修改有痕迹</a>"
										+ "→<a href='fzb_document_shenpidan_edit.jsp?action=read&d_templateid="
										+ d_templateid
										+ "&d_username="
										+ ms_userrName
										+ "&d_recordid="
								...
								s_Link = "<a href='fzb_document_shenpidan_edit.jsp?action=norevision&d_username="
										+ ms_userrName
										+ "&d_recordid="
										+ rs.getString("D_RecordID")
										+ "&caseId="
										...
										+ "→套红"
										+ "→签章"
										+ "→<a href='fzb_document_shenpidan_edit.jsp?action=read&d_username="
										+ ms_userrName
										+ "&d_recordid="
										+ rs.getString("D_RecordID")
										+ "&caseId="
