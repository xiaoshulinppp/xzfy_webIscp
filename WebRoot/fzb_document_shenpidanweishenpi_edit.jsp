...
<%
	...
	if (ms_Action.equals("new")) {
		...
		response.sendRedirect("fzb_default_shenpidanweishenpi_list.jsp?caseId="+ms_caseId+"&d_username"+ms_Author);
	}
		...
		<div id="bdy">
			...
			<hr />
						<%if(ms_NodeDesc.equals("阅读")){ %>
							...
							<% }else{%>
			<form id="form1" name="form1" method="post" action="fzb_document_shenpidanweishenpi_save.jsp?caseId=<%=ms_caseId%>&d_username=<%=ms_Author%>&d_checkstatus=<%=ms_checkstatus%>&userrid=<%=userrid%>"
				...
				<table class="">
							...
							<!--  <input type="button" class="btn4"
								onclick="location.href='fzb_default_shenpidanweishenpi_list.jsp?caseId=<%=ms_caseId%>&d_username=<%=ms_Author%>'" value="查看历史版本">-->
							...
							<!--  <input type="button" class="btn4"
								onclick="location.href='fzb_default_shenpidanweishenpi_list.jsp?caseId=<%=ms_caseId%>&d_username=<%=ms_Author%>'" value="返回列表">*/-->
							<br>
							<br>
...
function acceptAllRevisions2(){
	...
	function createApp(){
		...
		var options = {
			...
			url:'<%="fzb_document_shenpidanweishenpi_save.jsp?caseId="+ms_caseId+"&d_username="+ms_Author+"&d_checkstatus="+ms_checkstatus+"&userrid="+userrid%>', //提交给哪个执行 
			type:'POST',
			success: callbackfun
		};
