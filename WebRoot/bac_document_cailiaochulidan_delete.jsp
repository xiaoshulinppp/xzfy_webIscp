...
<%
	...
	if (!isNullOrEmpty(s_recordID)) {
		...
		try {
			...
			ExecuteUpdate(conn, s_sql);
			conn.commit();
			s_alert = "<script>alert('删除成功!');window.location='bac_default_cailiaochulidan_list.jsp?caseId="+ms_cassid+"&d_username="+ms_userrName+"';</script>";

		} catch (Exception ex) {
			s_alert = "<script>alert('删除失败!');window.location='bac_default_cailiaochulidan_list.jsp?caseId="+ms_cassid+"&d_username="+ms_userrName+"';</script>";
			conn.rollback();
		} finally {
			closeAll(conn, null, null);
		}
	} else {
		s_alert = "<script>alert('参数有误!');window.location='bac_default_cailiaochulidan_list.jsp?caseId="+ms_cassid+"&d_username="+ms_userrName+"';</script>";
	}
