...
function setValue(){
	node = this.object;
	parent.mainframe.location="./viewOrg.action?funcCode=${funcCode}&opName=${opName}&entityId="+node.id;
}
...
root = treeGlobe.createTreeNode("${org.name}",1,null,1);
root.id = ${org.id};
root.entityid = ${org.id};
root.entityabbr="${org.id}";
...
root.nameElement.onclick = function(){
  parent.mainframe.location="./viewOrg.action?funcCode=${funcCode}&entityId=${org.id}&opName=update";
}
