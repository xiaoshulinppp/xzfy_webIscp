...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="yourPlace"  >
  <tr>
         <td>${action.getText("current.location")}:${action.getText("orglist.title")}</td>
      </tr>
</table>
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" class="tabtitle">
  <tr>
    <td>${action.getText("orglist.title")}</td>
  </tr>
</table>
...
<table width="98%" border="0" align="center" cellpadding="3" cellspacing="0" class="tabout" id="color_table">
   ...
   <tr class="trcolor">
    ...
    <#if seq != 1>
     <td class="tdlistCenter"><a href ="../user/listOrg.action?funcCode=${funcCode}&parentId=${keyColumn}&backId=${parentId}">     <#if field?has_content>
         ${field}
       <#else>
         &nbsp;
       </#if></a></td>
    ...
    <#list displayInfo.operationList as op>
       <#if op.single !=0>
       <td class="tdlistCenter"><a href="../user/${op.abbr}Org.action?opName=${op.abbr}&entityId=${keyColumn}&funcCode=${funcCode}&pageNum=${pageNum}&parentId=${parentId}"><img src="../${Session["_USER_STYLE_"].imagePath}/sicons/${op.image}" alt="${op.name}" width="16" height="16" border="0"></a></td>
       </#if>
    </#list>
  </tr>
  </#list>
  ...
  <tr>
    <td><table  border="0" align="right" cellpadding="3" cellspacing="2">
      <tr align="center">
        ...
        <td align="center"><a href ="../user/listOrg.action?pageNum=1&funcCode=${funcCode}&parentId=${parentId}">${action.getText("firstpage")}</a></td>
         <#assign upPage = pageNum -1>
        <#if upPage <1 >
          <#assign upPage = 1>
        </#if>       
        <td align="center"><a href ="../user/listOrg.action?pageNum=${upPage}&funcCode=${funcCode}&parentId=${parentId}">${action.getText("uptopage")}</a></td>
        <#assign downPage = pageNum +1>
	
        <#if (downPage > totalPage)>
          <#assign downPage = totalPage >
        </#if>
        <td align="center"><a href ="../user/listOrg.action?pageNum=${downPage}&funcCode=${funcCode}&parentId=${parentId}">${action.getText("downtopage")}</a></td>
        <td align="center"><a href ="../user/listOrg.action?pageNum =${totalPage}&funcCode=${funcCode}&parentId=${parentId}">${action.getText("endpage")}</a></td>
...
<script>
  function uptopageiscp() {
   submitDisableButton();
  document.location.href="../user/listOrg.action?funcCode=${funcCode}&parentId=${backId}";
  }
  function FormSubmit(opName) {
     submitDisableButton();
     window.orgListForm.action="../user/"+opName+"Org.action?opName="+opName;
     window.orgListForm.submit();
  }
  ...
  function gotoPage(pMax) {
    ...
    var pLastNum = window.orgListForm.gotoPageNum.value ;
    ...
    window.location="../user/listOrg.action?pageNum="+pLastNum+"&funcCode=${funcCode}&parentId=${parentId}";
  }
</script>
