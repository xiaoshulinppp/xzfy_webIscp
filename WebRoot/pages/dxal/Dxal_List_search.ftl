...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" class="tabMainBot">
  <tr >
    <td>
        ...
        <a href ="../dxal/dxal_SearchList.action?pNum=1&funcCode=${funcCode}&title=<#if title?has_content>${title}</#if>&keyword=<#if keyword?has_content>${keyword}</#if>&case_title=<#if case_title?has_content>${case_title}</#if>&read_count=${read_count}&comment_count=${comment_count}">首页</a>
        <a href ="../dxal/dxal_SearchList.action?pNum=${upPage}&funcCode=${funcCode}&title=<#if title?has_content>${title}</#if>&keyword=<#if keyword?has_content>${keyword}</#if>&case_title=<#if case_title?has_content>${case_title}</#if>&read_count=${read_count}&comment_count=${comment_count}">上一页</a>	
        <a href ="../dxal/dxal_SearchList.action?pNum=${downPage}&funcCode=${funcCode}&title=<#if title?has_content>${title}</#if>&keyword=<#if keyword?has_content>${keyword}</#if>&case_title=<#if case_title?has_content>${case_title}</#if>&read_count=${read_count}&comment_count=${comment_count}">下一页</a>
        <a href ="../dxal/dxal_SearchList.action?pNum=${totalPage}&funcCode=${funcCode}&title=<#if title?has_content>${title}</#if>&keyword=<#if keyword?has_content>${keyword}</#if>&case_title=<#if case_title?has_content>${case_title}</#if>&read_count=${read_count}&comment_count=${comment_count}">末页</a>
        第
...
<script language="javascript">
			function gotoPage(){
				...
				if(inputPage > ${totalPage}){
					...
					window.location = "../dxal/dxal_SearchList.action?funcCode=${funcCode}&pNum=" + inputPage + "&title=<#if title?has_content>${title}</#if>&keyword=<#if keyword?has_content>${keyword}</#if>&case_title=<#if case_title?has_content>${case_title}</#if>&read_count=${read_count}&comment_count=${comment_count}";
				}
				return;
			}
</script> 
