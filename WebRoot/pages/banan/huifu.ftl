...
<script type="text/javascript" src="../${Session['_USER_STYLE_'].jsPath}/jquery.form.js"></script>  
	...
	<#assign isde=xzfyInfo.isdelay?default('0')>
			...
			<input type="hidden" id="checksize" value="${shenpiList2.size()}" />
...
<div class="align-right">
	...
	<#if user_id==user2_id || user_id==user3_id>
		<#if isView==1>
			<#if xzfyInfo.status == "8" || xzfyInfo.status == "9">
				<a href="javascript:saveToDocPrint()" title="" class="btn-blue btn">打印</a>
				<a href="javascript:saveToHuifuDoc()" title="" <#if xzfyInfo.check_status != "0">disabled</#if> class="btn-blue btn">恢复审理通知书</a>
			<#else>
				<a href="javascript:saveToDocPrint()" title="" class="btn-blue btn">打印</a>
				<#if dochuifuNum != 0>
				<a href="javascript:getHuifushenqingshu()" title="" <#if xzfyInfo.check_status != "0">disabled</#if> class="btn-blue btn">恢复审理通知书</a>			
				</#if>
			</#if>
		<#elseif isView==0>
			<a href="javascript:saveToDocPrint()" title="" class="btn-blue btn">打印</a>
			<#if dochuifuNum != 0>
			<a href="javascript:getHuifushenqingshu()" title=""  class="btn-blue btn">恢复审理通知书</a>
			</#if>
		</#if>
	<#else>
		<#if dochuifuNum != 0>
	...
	<div class="z_wrap_info" id="banan">
			...
			<h2 class="z_title"><font style="font-family:宋体;font-size:24px">${xzfyInfo.break_right?default('')}行政复议案件<br/>处理（恢复审理）呈报表</font></h2>
			<p align=right><font style="font-family:宋体;font-size:16px">${xzfyInfo.tjzfzh?default('')}</font></p>
			<div class="z_info">
		  	<table id="huifushenli"  width="100%" cellpadding="0" cellspacing="0" border="0" class="z_info_table">	  			
		  		<tr>
					<th width="18%">申请人</td>
				...
				<tr>
				<#assign stop_reason='    因中止原因消除，建议恢复案件审理。'>
				<#if xzfyInfo.stop_reason?has_content>
					<#assign stop_reason='    '+xzfyInfo.stop_reason?trim>
				</#if>
					<td colspan="4">
						...
						<div class="f12 c6">承办人意见：<br/>
							...
							<textarea onkeyup="checkLen(this)" name="stop_reason" type="text" class="textarea5" value="" id="stop_reason">${stop_reason}</textarea>
						...
						<div class="mt10 c6 f12">	
							<select name="" class="select-page" id="huifuspr" onchange="setChecker('${stop_reason}')">
							...
							<script>
								...
								function spr(){
									if(a==1){
										document.getElementById('huifuspr').style.display="block";
										a=0;}
									else{
										document.getElementById('huifuspr').style.display="none";
										a=1;}
								}
							</script>	
						...
						<#if user_id==user2_id || user_id==user3_id>		
							...
							<#else>
								<#if xzfyInfo.status?exists && xzfyInfo.status != "9" && xzfyInfo.status != "8" || xzfyInfo.check_status != "0" >
								<#else>
									<a href="javascript:tijiaoshenpi9()" class="btn-blue btn"  >提交审批</a>
								</#if>	
							</#if>
							
							<#if xzfyInfo.status=="9" && xzfyInfo.check_status != "1">
							<a href="javascript:save10()" class="btn-blue3 btn" <#if xzfyInfo.status?exists && xzfyInfo.status != "9" && xzfyInfo.status != "8" || xzfyInfo.check_status != "0" >disabled</#if> >完成</a>
							</#if>
							<#if isView==1>
							...
							<!--<#if xzfyInfo.huifuspdate?has_content><p align=right>${xzfyInfo.user2_name?default('')}:${xzfyInfo.huifuspdate?default('')}提交</p></#if>-->
							<p align=right>${xzfyInfo.user2_name?default('')}:<input size="10" style="font-family:宋体;font-size:17px" name="shenpiDate" id="shenpiDate" altname="提交审批日期" type="text" onClick="WdatePicker()"  value="<#if xzfyInfo.huifuspdate?exists>${xzfyInfo.huifuspdate?default('')}<#else>${todate}</#if>">提交</p>
						</div>
				...
				<tr>
					<td colspan="4" style="height:220px">
						<div class="mt10 c6 f12">领导批示：<br><br>
							<#list shenpiList2 as xzfyShenpi>
				...
				<tr>
					<td colspan="4">
						...
						<div class="mt10 em2">
							<textarea name="pifuyijian2" type="text" class="textarea2" value="" id="" >${xzfyInfo.pifuyijian2?default('')}</textarea>							
						</div>
			...
			<input type="hidden" id="pizhunren2" name="pizhunren2" />	  						
			</table><br/>
			</div>
		</div>			
	...
	function save10(){
		...
		//	return;
		//}
		//if(document.getElementById("huifuspdate").value==""){	
		//	alert("请输入审批日期");
		//	return;
		//}
		//if(document.getElementById("huifudate").value==""){	
		//	alert("请输入恢复日期");
		//	return;
		//}
		if(confirm("确认提交？")){
			document.getElementById("pizhunren2").value=document.getElementById("huifuspr").value;	
			window.form1.action="savebanan.action?tempstatus=10&tempcheckstatus=0&listkeyword=${listkeyword}";
			window.form1.submit();
			alert("操作已完成！");
		}
	}
	...
	function tijiaoshenpi9(){
		...
		if(confirm("确认提交？")){	
			...
			window.form1.action="saveshenpi.action?tempstatus=9&xzfyShenpi.check_type=9&tempcheckstatus=1&listkeyword=${listkeyword}";	
			window.form1.submit();
		}
	}
	...
	function getHuifushenqingshu(){
		...
		if(isView=="0")
		{
			var url ="../fzb_default_dafutongzhi.jsp?&d_filetype=doc&d_username=${Session['_USER_LOGIN_'].id}&caseId=${xzfyInfo.id}&mstype=huifushenlitongzhishu&actiontype=yuedu";
		}
		else{
			var url ="../fzb_default_dafutongzhi.jsp?&d_filetype=doc&d_username=${Session['_USER_LOGIN_'].id}&caseId=${xzfyInfo.id}&mstype=huifushenlitongzhishu&actiontype=1";
		}
...
function saveToDocPrint(){
	...
	function docPrint(){
			...
			var url="../fzb_default_shenpidanweishenpi.jsp?d_filetype=doc&d_username=${Session['_USER_LOGIN_'].id}&caseId=${xzfyInfo.id}&d_checkstatus=9";
		else
		var url="../fzb_default_shenpidan.jsp?d_filetype=doc&d_username=${Session['_USER_LOGIN_'].id}&caseId=${xzfyInfo.id}&d_checkstatus=9";
		...
		window.open(url,'mainwindow2','location=no,resizable=yes');
			  	...
			  	for(var i=0;i<subcat.length;i++){
			  		if(document.getElementById("huifuspr").value==subcat[i][0]){
			  				document.getElementById("stop_reason").value=document.getElementById("stop_reason").value+"请"+subcat[i][1]+"同志阅示。";
			  	}
			  }
	}
