					...
					<#list dftzList as xzfyDftz>
						<tr class="trcolor">
							...
							<td  id="cxs" style="display:block" >
								<a href="http://localhost:8080/webIscp/bac_default_dafutongzhi.jsp?&d_filetype=doc&d_username=${Session['_USER_LOGIN_'].id}&caseId=${xzfyInfo.id}";">查看通知</a>&nbsp;		
							</td>
							<td  id="cxs" style="display:block" >
								<#if xzfyDftz.status?has_content && xzfyDftz.status=="0">
									<a href="http://localhost:8080/webIscp/bac_default_dafutongzhi.jsp?&d_filetype=doc&d_username=${Session['_USER_LOGIN_'].id}&caseId=${xzfyInfo.id}">查看答复</a>&nbsp;		
								<#else>未答复
								</#if>
							</td>
							<td  >
								<#if xzfyDftz.fujian11?has_content>
								<a href="<#if xzfyDftz.fujian11?has_content>${xzfyDftz.fujian11}</#if>" target="_blank" >证明材料1</a>
								<#else>未上传
								</#if>
							</td>	
							<td  >
								<#if xzfyDftz.fujian22?has_content>
								<a href="<#if xzfyDftz.fujian22?has_content>${xzfyDftz.fujian22}</#if>" target="_blank" >证明材料2</a>
								<#else>未上传
								</#if>
							</td>
							<td  >
								<#if xzfyDftz.fujian33?has_content>
								<a href="<#if xzfyDftz.fujian33?has_content>${xzfyDftz.fujian33}</#if>" target="_blank" >证明材料3</a>
								<#else>未上传
								</#if>
							</td>
						</tr>
