    ...
    <td><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
      <tr>
          ...
          <#list displayInfo.operationList as op>
            <#if op.single!=1>
              <#if op.abbr == "create">
              <input id ="${op.abbr}Id" name="Button" type="button" class="button" value="${op.name}" onclick="MM_openBrWindow('createDictItemNew.action?funcCode=${funcCode}&dictId=${dictId}&parentId=${parentId}&opName=create','creatSub','status=yes,scrollbars=yes,resizable=yes,width=550,height=350')">
              <#else>
...
<table width="98%" border="0" align="center" cellpadding="3" cellspacing="0" class="tabout" id="color_table">
   ...
   <tr class="trcolor">
    ...
    <#list displayInfo.operationList as op>
       <#if op.single!=0 >
       <td class="tdlistCenter"><a href="../system/${op.abbr}DictItem.action?opName=${op.abbr}&entityId=${keyColumn}&funcCode=${funcCode}&pageNum=${pageNum}&dictId=${dictId}&parentId=${parentId}"><img src="../${Session["_USER_STYLE_"].imagePath}/sicons/${op.image}" alt="${op.name}" width="16" height="16" border="0"></a></td>
       </#if>
    </#list>
  </tr>
  </#list>
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" class="tabMainBot">
  <tr>
    <td>
        ...
        <a href ="../system/listDictItem.action?pageNum=1&funcCode=${funcCode}&dictId=${dictId}&parentId=${parentId}">${action.getText("firstpage")}</a>
         <#assign upPage = pageNum -1>
        <#if upPage <1 >
          <#assign upPage = 1>
        </#if>      
        <a href ="../system/listDictItem.action?pageNum=${upPage}&funcCode=${funcCode}&dictId=${dictId}&parentId=${parentId}">${action.getText("uptopage")}</a>
        <#assign downPage = pageNum +1>
	
        <#if (downPage > totalPage)>
          <#assign downPage = totalPage >
        </#if>
        <a href ="../system/listDictItem.action?pageNum=${downPage}&funcCode=${funcCode}&dictId=${dictId}&parentId=${parentId}">${action.getText("downtopage")}</a>
        <a href ="../system/listDictItem.action?pageNum =${totalPage}&funcCode=${funcCode}&dictId=${dictId}&parentId=${parentId}">${action.getText("endpage")}</a>
        ${action.getText("di")}
...
<script>
  function FormSubmit(opName) {
     submitDisableButton();
     window.dictItemListForm.action="../system/"+opName+"DictItem.action?opName="+opName;
  ...
  function gotoPage(pMax) {
    ...
    window.location="../system/listDictItem.action?pageNum="+pLastNum+"&funcCode=${funcCode}&dictId=${dictId}&parentId=${parentId}";
  }
