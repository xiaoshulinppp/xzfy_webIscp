...
<table border="0" cellspacing="0" cellpadding="0" class="tdMain2j">
  <tr>
    <td class="td2jYourYlace">您的位置:日志管理 --&gt 查询列表</td>
  </tr>
  <tr>
    <td height="4"></td>
  </tr>
...
<tr>
	...
	<tr class="trcolor">
	    ...
	    <#assign seq = seq + 1>
	 	...
	 	<#-- 操作 -->
	    <td align="center" class="tdlistCenter"><a href="#"><img src="../images/16B/sicons/search_new.gif" alt="${"查看"}" width="16" height="16" border="0" onclick="fnView(${keyColumn})"></a></td>
	</tr>
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="tabMainBot">
  ...
  <#assign totalPage = displayInfo.totalPage >
    <td>
        ...
        <a href ="../system/queryAppLog.action?funcCode=${funcCode}&type=${type}&pageNum=1">首页</a>
         <#assign upPage = pageNum -1>
        <#if upPage <1 >
          <#assign upPage = 1>
        </#if>       
       <a href ="../system/queryAppLog.action?funcCode=${funcCode}&type=${type}&pageNum=${upPage}">上一页</a>
        <#assign downPage = pageNum +1>
	
        <#if (downPage > totalPage)>
          <#assign downPage = totalPage >
        </#if>
        <a href ="../system/queryAppLog.action?funcCode=${funcCode}&type=${type}&pageNum=${downPage}">下一页</a>
        <a href ="../system/queryAppLog.action?funcCode=${funcCode}&type=${type}&pageNum =${totalPage}">末页</a>
        第
...
<script>
  	function gotoPage(pMax) {
    	...
    	window.location="../system/queryAppLog.action?funcCode=${funcCode}&type=${type}&pageNum="+window.listForm.gotoPageNum.value;
  	}
	function fnView(id){
		openwindowMiddle('../system/appLogQueryView.action?entityId='+id+'&funcCode=${funcCode}&pageNum=${pageNum}&type=${type}');
	}	
</script>
</form>
</body>
