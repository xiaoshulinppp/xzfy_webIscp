...
function setValue(){
	node = this.object;
	parent.mainframe.location="./viewPtree.action?postId=${entityId}&funcCode=${funcCode}&opName=${opName}&entityId="+node.id;
}
serverUrl = "./showPtreeServer.action?funcCode=${funcCode}";

//
treeGlobe.treeNodeElement = document.getElementById("tree_node");
root = treeGlobe.createTreeNode("${position.name}",1,null);
root.id = 0;
...
root.nameElement.onclick = function(){
  parent.mainframe.location="./createPtree.action?funcCode=${funcCode}&postId=${entityId}&parentId=0&opName=create";
}
