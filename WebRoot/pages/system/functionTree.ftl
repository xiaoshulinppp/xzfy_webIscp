...
function setValue(){
	node = this.object;
	parent.mainframe.location="./updateFunction.action?funcCode=${funcCode}&opName=${opName}&entityId="+node.id;
}
serverUrl = "./showFunctionServer.action?funcCode=${funcCode}";

//
treeGlobe.treeNodeElement = document.getElementById("tree_node");
root = treeGlobe.createTreeNode("功能树",0,null);
root.id = 0;
root.entityid = 0;
root.entityabbr="";
...
root.nameElement.onclick = function(){
  parent.mainframe.location="./createFunction.action?funcCode=${funcCode}&parentId=0&opName=create";
}
