...
inputElement = opener.document.getElementById("${inputElementId}");
showElement = opener.document.getElementById("${showElementId}");
<#if multiSelect == "true">
function setValue(){
	name = "";
	value = "";
...
function setValue(){
	node = this.object;
	showElement.value = node.value;
	inputElement.value = node.name;
	window.close();
}
</#if>
serverUrl = "./selectAllDictItem.action?funcCode=${funcCode}&entityId=${entityId}";
