...
<div class="fl w198">
      <div class="sideNav">
            ...
            <ul>
                 <li><a href="javascript:;" <#if xzfyInfo.status=="2"> <#else> class="aNow" </#if>>案件信息</a></li>
                  <#if xzfyInfo.isBuzheng=="1">
                  	<#if xzfyInfo.status=="2">
                  		<li><a href="javascript:;" class="aNow">补正审批</a></li>
                  	<#else>
                  		<li><a href="javascript:;" >查看补正程序</a></li>
                  	</#if>
                  </#if>
                  <#if xzfyInfo.status=="2">
                  		<li><a href="javascript:;" class="aNow">补正审批</a></li>
                  ...
                  <#if xzfyInfo.status=="1" && xzfyInfo.isBuzheng=="0">
                  	<li><a href="javascript:;" >申请补正</a></li>
                  </#if>
                 <li><a href="javascript:dyniframesize('ifm');">接待信息</a></li>
            </ul>
      </div>
...
function getBzPro(status){
	...
	function checkBzConfirm_ja(){
		if(document.getElementById("jiean_check_id").value==0){
			document.getElementById("jaFast").style.display="";
		}else{
...
<div class="fr w739">
      <div class="pageNow">
当前位置：&nbsp;&nbsp;<a href="../xzfy/gotoIndex.action?funcCode=${funcCode}">首页  </a> >&nbsp;&nbsp;  <a href="../banan/showanjianbanliList.action?listkeyword=1&funcCode=${funcCode}">待办案件</a>  >&nbsp;&nbsp; 案件办理
      
      </div>
   <#assign checkUserId=xzfyInfo.check_id?default('0')>
<div class="pageDiv" <#if xzfyInfo.status=="2">style="display:none" <#else>style="display:block" </#if>>
...
<table  id="shouli_tr1" class="ui-table ui-table-form "  
  ...
  <tr >
  <th width="25%">立案审查人意见:</th>
  <td width="25">
  <textarea class="textarea" name="xzfyInfo.laAdvice" >${xzfyInfo.laAdvice?default('')}</textarea>
  </td>
  <th width="25%">立案日期:</th>
  <td width="25%"><input class="input2"  onClick="WdatePicker()"   name="xzfyInfo.lian_date" id="lian_date" value="${xzfyInfo.lian_date?default('')}"></td>
  </tr>
  <tr >
	<th >立案审批人：</th>
    <td colspan="2">
...
<th>案号：</td>
<td >
<input name="xzfyInfo.tjzfzh"  value="${xzfyInfo.tjzfzh?default('复字[201]号')}"  id="tjzfzh" class="input2" class="inputTextNormal"  >
</td>
<th>&nbsp;</td>
<td class="tdFormInput">
 ...
 <tbody id="laFast"  <#if xzfyInfo.status=='1' || (la_checkSize==0 && isShowBtn=='0') || (xzfyInfo.status=='3' && isShowBtn=='0')>style="display:block"<#else>style="display:none"</#if> >
  <tr >
    <th>立案批准人：</th>
    <td class="tdFormInput2"><input class="input2" name="lashenpiren" id="lashenpiren">
      </td>
	<th>批准意见：</th>
    <td class="tdFormInput2"><textarea class="textarea" name="lashenpiyijian" id="lashenpiyijian"></textarea>
      </td>
  </tr>
</tbody>
  <tr >
   <td colspan="4" align="center" padding-left:17px;>

	 <#if xzfyInfo.status=="1" || xzfyInfo.status=="3">
		<#if isShowBtn=='0'>
	           &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <input class="btn4" type="button"  value="提交审批" onclick="subLian()">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	            <input class="btn4" type="button"  value="完成立案" onclick="finishLian('${checkSize}')">
		<#else>
			 <input class="btn4" type="button" id="cancleBz"  value="取消审批" onclick="cancleLian()">
		</#if>
	</#if>
...
<!--结案信息-->
 ...
 <#assign finish_type2=xzfyInfo.finishBreakType?default('0')>
    <tr class="trcolor">
     <th class="tdFormText" width="25%">结案方式：</td>
     <td class="tdFormInput" width="25%">
         <select name="xzfyInfo.finish_type" id="finishtype1" class="select" onchange="setFT()">
		  ...
		  <#if finish_type1=='8'>style="display:block"<#else>style="display:none"</#if> altname="结案方式" class="select" id="finishtype2" ishidden="y">
		  <option value="0">----请选择----</option>
	<option value="1" <#if finish_type2=='1'>selected</#if>>和解协议</option>
	<#-- <option value="2" <#if finish_type2=='2'>selected</#if>>撤回申请</option>-->
	 <option value="5" <#if finish_type2=='5' || finish_type2=='2' ||finish_type2=='3'>selected</#if>>撤回申请</option>
  ...
  <td width="30%">
  <textarea class="textarea" name="xzfyInfo.jaAdvice" class="textarea">${xzfyInfo.jaAdvice?default('')}</textarea>
  </td>
  <th width="20%">结案日期:</th>
  <td width="30%"><input class="input2"  onClick="WdatePicker()"   name="xzfyInfo.finish_date" id="finish_date" value="${xzfyInfo.finish_date?default('')}"></td>
  </tr>
  <tr >
	<th >结案审批人：</th>
    <td colspan="2">
...
<#if ja_checkSize==0 >
 ...
 <tbody id="jaFast"  <#if (xzfyInfo.status=='11' && isShowBtn=='0') || (ja_checkSize==0 && isShowBtn=='0') >style="display:''"<#else>style="display:none"</#if> >
  <tr >
    <th>结案批准人：</th>
    <td class="tdFormInput2"><input class="input2" name="jashenpiren" id="jashenpiren">
    <br><span id="lianSprShow2"></span>
      </td>
	<th>批准意见：</th>
    <td class="tdFormInput2"><textarea class="textarea" name="jashenpiyijian" id="jashenpiyijian"></textarea>
      </td>
  </tr>
</tbody>
  ...
  <tr >
   <td colspan="4" align="center">
	 <#if xzfyInfo.status=="1" || xzfyInfo.status=="11">
		<#if isShowBtn=='0'>
	            <input class="btn4" type="button"  value="提交审批" onclick="subJiean()">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	            <input class="btn4" type="button"  value="完成结案" onclick="finishJiean('${checkSize}')">
		<#else>
			 <input class="btn4" type="button" id="cancleBz"  value="取消审批" onclick="cancleJiean()">
		</#if>
	</#if>
    </td> 
...
<input name="xzfyInfo.TApplicationDeptID" type="hidden" class="inputTextNormal" id="deptid" />
 <table  ><!--表单填写项文字居右样式由 fn-text-right控制。td宽度在第一行标记 不要每行都标记-->
  ...
  <tr class="trcolor" id="handleCase" <#if xzfyInfo.status=='2'>style="display:none"</#if>>
	...
	<th>来件处理：</td>
	    <td class="tdFormInput">
<select name="scresult" class="select"
...
<td>&nbsp;</td>
   ...
   <tr >
    <th  width="25%">申请人：</td>
    <td width="25%">
    <textarea class="textarea" name="xzfyInfo.appdetail" class="textarea" id="user" ischeck="y" ><#if xzfyInfo.appdetail?has_content>${xzfyInfo.appdetail}</#if></textarea></td>
    <th  width="25%">收案日期：</td>
    <td width="25%">
    <input name="xzfyInfo.receive_date" id="receivedate" ischeck="y"  altname="收案日期"  readonly
    class="input2" onClick="WdatePicker()"  <#if xzfyInfo.receive_date?has_content>value="${xzfyInfo.receive_date}"</#if>>
      </td>
  </tr>

   <tr class="trwhite">
    <th>申请人总数：</td>
    <td><input name="xzfyInfo.renCount" class="input2" id="rencount" ischeck="y"  class="inputTextNormal" altname="申请人总数" <#if xzfyInfo.renCount?has_content>value="${xzfyInfo.renCount}"</#if>></td>
	<th>&nbsp;</td>
    <td>&nbsp;</td>
   </tr>
   ...
   <tr class="trcolor">
    ...
    -->
     ...
     <th>立案审查人姓名：</td>
    <td class="tdFormInput"><input name="xzfyInfo.user1_name" id="checker" class="input2" ischeck="y" readonly altname="立案审查人姓名" class="inputTextNormal"  <#if xzfyInfo.user1_name?has_content>value="${xzfyInfo.user1_name}"</#if>></td>
     ...
     <tr class="trwhite">
    <th>代表人名称：</td>
    <td class="tdFormInput"><input name="xzfyInfo.deputy_show" class="input2" class="inputTextNormal" <#if xzfyInfo.deputy_show?has_content>value="${xzfyInfo.deputy_show}"<#else>value="无"</#if>> </td>
   <th>第三人：</td>
    <td class="tdFormInput"><input name="xzfyInfo.third" class="input2" class="inputTextNormal"  <#if xzfyInfo.third?has_content>value="${xzfyInfo.third}"<#else>value="无"</#if>></td>
   </tr>   
   
   <tr class="trwhite">
    <th>代理人：</td>
    <td class="tdFormInput"><input name="xzfyInfo.agent_name" class="input2" id="wtdlr" class="inputTextNormal"  <#if xzfyInfo.agent_name?has_content>value="${xzfyInfo.agent_name}"<#else>value="无"</#if>></td>
   <th>第三人代理人：</td>
    <td class="tdFormInput"><input name="xzfyInfo.third_agent" class="input2" class="inputTextNormal"  <#if xzfyInfo.third_agent?has_content>value="${xzfyInfo.third_agent}"<#else>value="无"</#if>></td>
   </tr>
   ...
   <th>复议机关类型：</td>
    <td class="tdFormInput">
	    <select name="xzfyInfo.xzfy_org" ischeck="y" altname="复议机关" class="select">
  ...
  <tr class="trcolor">
    ...
    <th>被申请人名称：</th>   
     <td > 	     	     
	    <span  style="display:none;" id="beappnametd" >
	     <input name="ssss" class="input2" class="inputTextNormal" id="beappnameinput" altname="被申请人名称"> 
   ...
   altname="申请复议事项" class="select" id="xzfysx2xzsx"
    ...
    <#assign neirong=''>
    <textarea class="textarea" name="xzfyInfo.xzfy_requset" id="xzfyqq1" class="input2" 
...
<#else>${qingqiu}</#if></textarea></td>
 ...
 <th>被复议的具体行政行为名称：</td>
    <td class="tdFormInput">
	    ...
	    <input name="xzfyInfo.xwname" class="input2" id="TBeName" value="${xzfyInfo.xwname}"
	     ischeck="y" altname="被复议的具体行政行为">
	     <#else>
			 	<input name="xzfyInfo.xwname" class="input2" id="TBeName" ischeck="y"
    ...
    <td class="tdFormInput">
	    ...
	    <input name="xzfyInfo.xwnum" class="input2" id="annum" value="${xzfyInfo.xwnum}"
	     ischeck="y" altname="被复议的具体行政行为案号">
	     <#else>
			 	<input name="xzfyInfo.xwnum" class="input2" id="annum" ischeck="y"
...
<#--
  <tr class="trwhite">
     <th>审查人意见：</td>
    <td class="tdFormInput" colspan=3><input name="xzfyInfo.firstSubmitResult" id="cbryj" class="input2" class="inputTextNormal" size=100></td>
   </tr>
  <tr class="trwhite"> 
      <th>审查人提交日期：</td>
    <td class="tdFormInput"><input name="xzfyInfo.firstSubmitDate" id="cbrtjrq" ischeck="y" altname="审查人提交日期" onClick="WdatePicker()" class="input2" class="inputTextNormal">
     <th>&nbsp;</td>
    <td class="tdFormInput">&nbsp;</td>
  </tr>
  -->
  ...
  <tr class="trcolor">
    <th>简要内容：<br>（限200字）</td>
    <td class="tdFormInput">
<textarea class="textarea" class="textarea" name="xzfyInfo.content_abstract" id="content"><#if xzfyInfo.content_abstract?has_content>${xzfyInfo.content_abstract}
...
<#else>${xzfyInfo.xzfy_requset?default("")}
	...
	<th>备注：</td>
    <td class="tdFormInput">
<textarea class="textarea" class="textarea" name="xzfyInfo.remark" id="remark"></textarea>
      </td>
  </tr>
 ...
 <div class="btn">
                       <div class="fc">
                       <a href="javascript:saveShencha()">保存</a>
 ...
 <#if xzfyInfo.isBuzheng=="1" || xzfyInfo.status=="2" || xzfyInfo.status=="1">

 	<div class="pageDiv"  <#if xzfyInfo.status=="2">style="display:block" <#else>style="display:none" </#if>>
            <!---->
            <div class=""></div>
             <div class="formDiv">
  <table   >
   <tr >
  	<th width="25%">通知补正日期：</th>
    <td width="25%">
 	   <input name="xzfyInfo.bzTongzhiriqi" id="addbegin" class="input2" onClick="WdatePicker()" altname="通知补正日期"
 	     bzcheck="y" ${xzfyInfo.bzTongzhiriqi?default('')}>
    </td>
    <th width="25%">补齐材料日期:</th>
    <td width="25%">
    <input name="xzfyInfo.bzBuqiriqi" id="addend" class="input2" onClick="WdatePicker()"
    ${xzfyInfo.bzBuqiriqi?default('')} altname="补正截止日期" >
    </td>
    </tr>
    <tr>
    <th >补正内容：</th>
    ...
    <textarea class="textarea"  class="textarea" name="xzfyInfo.jdBzContent" id="bznr" ><#if xzfyInfo.jdBzContent?has_content>${xzfyInfo.jdBzContent}<#else>该行政复议申请中有如下材料欠缺或表达不清，根据《中华人民共和国行政复议法实施条例》第29条的规定，请申请人于接待之日起5个工作日内补正。无正当理由逾期不补正的，视为放弃行政复议申请。补正申请材料所用时间不计入行政复议案件审理期限。</#if></textarea>
    </td>
    </tr>
    <tr>
    <th>立案审查人意见：</th>
    <td colspan=3>
    <textarea class="textarea" name="xzfyInfo.bzAdvice" id="bzAdvice" ><#if xzfyInfo.bzAdvice?has_content>${xzfyInfo.bzAdvice}</#if></textarea>
    </td>
    </tr>
...
<tr >
    <th>补证批准人：</td>
    <td ><input class="input2" name="shenpiren" id="shenpiren">
    <br><span id="seshenpiren"></span>
      </td>
	<th>&nbsp;</th>
    <td class="tdFormInput2">&nbsp;
    <!--<input class="input2" name="shenpishijian" id="shenpishijian" onClick="WdatePicker()" class="inputTextNormal">-->
      </td>
  </tr>
  <tr >
    <th>批准意见：</th>
    <td colspan=3>
    <textarea class="textarea"  name="shenpiyijian" class="textarea" id="shenpiyijian"></textarea>
      </td>
  </tr>
</tbody>
</table>
<br>
 ...
 <div class="btn">
		...
		<#if isShowBtn=='0'>
                       <a href="javascript:subBuzheng()">提交审批</a>
                       <a href="javascript:finishBuzheng('${checkSize}')">完成补正</a>
	<#else>
			<a href="javascript:cancleBuzheng()">取消审批</a>
		</#if>
 ...
 <div class="pageDiv" >
      <iframe frameborder="0" id="ifm" name="ifm" onload="javascript:dyniframesize('ifm');"    scrolling="no" width="100%" src="../xzfy/shenpiFormInfo.action?funcCode=${funcCode}&xzfyId=${xzfyId}&checkDocType=3"></iframe>
	  </div>
</div>
...
function saveShencha(){
	...
	if(confirm("确定要保存吗？"))
		...
		listForm_.action="../xzfy/saveLianshencha1.action?funcCode=${funcCode}&xzfyId=${xzfyInfo.id}";
