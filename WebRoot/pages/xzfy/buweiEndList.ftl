...
<form name="Form1" id="Form1" action="../xzfy/buweiEnd.action" method="post">
      ...
      <div class="pageBox">
            ...
            <table width="100%" border="0" cellpadding="0" cellspacing="0">
				...
				<#list xzfyBuweiList as buwei>
					<tr>
						...
						<#if buwei.appType=="1">
						<#assign app_show=buwei.appDetail?default('无')>
						<td title='${app_show}'>&nbsp;<#if app_show.length() gt 26>${app_show[0..25]}..<#else>${app_show?default('')}</#if></td>
						<#else>
						<#assign app_show=buwei.agentDetail?default('无')>
						<td title='${app_show}'>&nbsp;<#if app_show.length() gt 26>${app_show[0..25]}..<#else>${app_show?default('')}</#if></td>
						</#if>
            ...
            <div class="pageNum">
               第${pageNum}/${totalPage}页   共${xzfyBuweiList.size()}条
				...
				<a href="../xzfy/buweiEnd.action?pageNum=1&funcCode=${funcCode}" class="ui-page-item ui-page-item-first">首页</a>
			    <a href="../xzfy/buweiEnd.action?pageNum=${upPage}&funcCode=${funcCode}" class="ui-page-item ui-page-item-prev">上一页</a>
		     	<a href="../xzfy/buweiEnd.action?pageNum=${downPage}&funcCode=${funcCode}" class="ui-page-item ui-page-next">下一页</a>
			    <a href="../xzfy/buweiEnd.action?pageNum=${totalPage}&funcCode=${funcCode}" class="ui-page-item ui-page-last">尾页</a>　
...
<script>
		function tiqu(id) {
			window.location = "../xzfy/buweiEndView.action?funcCode=${funcCode}&xzfyId="+id;
		}
		...
		function gotoPage(){
				...
				if(inputPage > ${totalPage}){
					...
					window.location = "../xzfy/buweiEnd.action?funcCode=${funcCode}&pageNum=" + inputPage;
				}
				return;
			}
