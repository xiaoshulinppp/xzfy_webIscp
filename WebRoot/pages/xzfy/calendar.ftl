...
function changeCld() {
	...
	m=CLD.SM.selectedIndex;
	 ...
	 var u = "getCalendar.action?funcCode=${funcCode}&years="+y+"&months="+m;
	    // alert(u);
		window.location= u;
}
...
function gotoVacation() {
	...
	m=CLD.SM.selectedIndex;
	 ...
	 var u = "chooseXiuXiRi.action?funcCode=${funcCode}&years="+y+"&months="+m;
	    // alert(u);
		window.open(u, 'newwindow', 'height=300, width=450, top=200,left=700, toolbar=no, menubar=no, scrollbars=no, resizable=no,location=no, status=no');
}
//-->
</SCRIPT>
<script language="javascript">
 ...
 var s,festival,txt='';
	...
	if(sObj.innerHTML!='') {
		if(cld[d].txtFestival == ''){ 
			...
			if(tempTxt!=null && tempTxt!=""){
				var lineTxt = tempTxt.split("@@");
				for(var m=0;m<lineTxt.length-1;m++){
				var content=lineTxt[m].split("$$");
				if(content[0]!=0){
				
				}else{
				txt += (m+1)+'.' + content[1] + '<br>';
				}
		...
		title = cld[d].sYear+'年'+cld[d].sMonth+'月'+cld[d].sDay+'日';
	}	

	if(msg!=''&&sObj.innerHTML!=''){
		 var titleheight = "55px"; // 提示窗口标题高度 
		 var bordercolor = "#EEC900"; // 提示窗口的边框颜色 
		 var titlecolor = "#000000"; // 提示窗口的标题颜色 
		 var titlebgcolor = "#FFA54F"; // 提示窗口的标题背景色
		 var bgcolor = "#FFFFFF"; // 提示内容的背景色
		 var um=document.getElementById("um");
		 var   x,y;   
		   oRect= um.getBoundingClientRect();   
		
		 var iWidth = oRect.left; 
		 var iHeight = oRect.top; 
		//iWidth=320;
		//iHeight=320;
		iWidth=iWidth+353;
		iHeight=iHeight+253;
		...
		//alert(iWidth);
		 ...
		 bgObj.style.cssText = "position:absolute;left:0px;top:0px;width:"+iWidth+"px;height:"+Math.max(document.body.clientHeight, iHeight)+"px;filter:Alpha(Opacity=0);opacity:0.3;background-color:#000000;z-index:101;";
		 document.body.appendChild(bgObj); 
		 
		 var msgObj=document.createElement("div");
		 msgObj.style.cssText = "position:absolute;font:12px '宋体';top:"+(iHeight-h)/2+"px;left:"+(iWidth-300)/2+"px;width:"+w+"px;height:"+h+"px;text-align:center;border:1px solid "+bordercolor+";background-color:"+bgcolor+";padding:1px;line-height:22px;z-index:102;";
		 ...
		 titleBar.style.cssText = "width:270px;height:25px;text-align:left;padding-top:5px;margin:0px;font:bold 12px '宋体';color:"+titlecolor+";border:0px solid " + bordercolor + ";cursor:move;background-color:" + titlebgcolor;
		 ...
		 closeBtn.style.cssText = "width:30px;cursor:pointer; padding-right:6px;background-color:" + titlebgcolor;
		 closeBtn.innerHTML = "<span style='font-size:9pt; color:"+titlecolor+";'>关闭</span>";
		 closeBtn.onclick = function(){ 
		  document.body.removeChild(bgObj); 
		  document.body.removeChild(msgObj); 
		 } 
		 var msgBox = table.insertRow(-1).insertCell(-1);
		 msgBox.style.cssText = "font:12pt '微软雅黑';color:#5a752d;text-align:left;";
		 msgBox.colSpan  = 2;
...
<input type="hidden" value="${time}" id="timeCurrent">
<input type="hidden" value="${timem}" id="timeCurrentM">
  ...
  <TR>
    <TD >
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="contTitleMonth">
		    ...
		    <td><a target='_blank' href="../xzfy/getWordList.action?funcCode=${funcCode}">工作记录</a></td>
		    <td><a href="javascript:void(0)" onclick="gotoVacation();">选择休息日</a></td>
		    </tr>
		  </table>
        ...
        <SCRIPT language=JavaScript>
            ...
            for(i=0;i<6;i++) {
               ...
               for(j=0;j<7;j++) {
                  gNum = i*7+j
                  document.write('<td  id="GD' + gNum +'" onclick="alertWin(' + gNum +')"><font id="SD' + gNum +'" style="font-size:12px;" face="Arial Black"')
