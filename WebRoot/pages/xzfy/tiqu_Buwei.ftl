   ...
   <table width="95%">
     <tr>
    	...
    	<td  width="30%">
		    ...
		    id="shoulijiguan" ischeck="y" altname="来信人姓名" value="${xzfyBuwei.shouLiJiGuan?default('')}"  >
    	<th  width="20%">收案日期：</th>
    	<td  width="30%">
		    <input name="xzfyBuwei.receiveDate" type="text"
		     id="receivedate" class="input2" ischeck="y"
		      onClick="WdatePicker()" altname="接收日期" value="${xzfyBuwei.receiveDate?default('')}">
      	</td>
  	</tr>
    ...
    <input type="hidden" id="rencount" value="0">
  	<tr>
  		<th>申请人类别：</th>
  		<td><@ww.select  name="'xzfyBuwei.appType'" id="applicationtype" headerKey="''" headerValue=""  list="getDictMap('sqrlb')" value="" onchange="'checksqr1()'" value="'${xzfyBuwei.appType?default('')}'.toString()"/></td>
  		<th></th>
  		<td></td>
  	</tr>
  	...
  	<th >申请人：</th>
    <td  colspan=3>
    <textarea name="xzfyBuwei.appDetail" type="text" class="textarea"  id="shenqingren" ischeck="y" altname="申请人" style="overflow-y:visible;"  >${xzfyBuwei.appDetail?default('')}</textarea>
    </td>
   </tr>
   <tr class="trwhite" id="sqr" <#if xzfyBuwei.appType=="1"> style="display:none"</#if>>
    <th>申请人名称：</th>
    <td colspan=3>
    <textarea name="xzfyBuwei.agentDetail" type="text" id="appname" class="textarea" ischeck="y" altname="法人" style="overflow-y:visible;" >${xzfyBuwei.agentDetail?default('')}</textarea>
    </td>
   </tr>
  	<tr>
  		<th>具体行政行为名称：</th>
  		<td><textarea name="xzfyBuwei.xwName" type="text" class="textarea" id="xwName" ischeck="y" altname="申请人" style="overflow-y:visible;"  >${xzfyBuwei.xwName?default('')}</textarea></td>
  		<th>具体行政行为文号：</th>
  		<td><textarea name="xzfyBuwei.xwNum" type="text" class="textarea" id="xwNum" ischeck="y" altname="申请人" style="overflow-y:visible;"  >${xzfyBuwei.xwNum?default('')}</textarea></td>
  	</tr>
 	<tr>
 		<th>行政复议事项：</th>
 		<td><@ww.select id="request2yiban" headerKey="'0'" headerValue="'--请选择--'"  list="getDictMap('xzfyqq')" onchange="'ryq();'"/>
   
    <textarea name="xzfyBuwei.xzfyRequest" id="xzfyqq1" type="text"  style="overflow-y:visible;" class="textarea" ischeck="y" altname="行政复议请求" >${xzfyBuwei.xzfyRequest?default('')}</textarea>
  </td>
 		<th>行政管理类别：</th>
 		<td><select name="xzfyBuwei.refer" title="该信息由立案审查人填写，请根据实际情况慎重修改"  id="ajsjfm1" ischeck="y" altname="行政管理类别" >
	<option value="0">----请选择----</option>
	<option value="1" <#if xzfyBuwei.refer?default('0')=="1">selected</#if>>公安</option>
	<option value="2" <#if xzfyBuwei.refer?default('0')=="2">selected</#if>>国家安全</option>
	<option value="3" <#if xzfyBuwei.refer?default('0')=="3">selected</#if>>劳动和社会保障</option>
	<option value="4" <#if xzfyBuwei.refer?default('0')=="4">selected</#if>>司法行政</option>
	<option value="5" <#if xzfyBuwei.refer?default('0')=="5">selected</#if>>民政</option>
	<option value="6" <#if xzfyBuwei.refer?default('0')=="6">selected</#if>>土地</option>
	<option value="7" <#if xzfyBuwei.refer?default('0')=="7">selected</#if>>地矿</option>
	<option value="8" <#if xzfyBuwei.refer?default('0')=="8">selected</#if>>环保</option>
	<option value="9" <#if xzfyBuwei.refer?default('0')=="9">selected</#if>>农业</option>
	<option value="10" <#if xzfyBuwei.refer?default('0')=="10">selected</#if>>水利</option>
	<option value="11" <#if xzfyBuwei.refer?default('0')=="11">selected</#if>>林业</option>
	<option value="12" <#if xzfyBuwei.refer?default('0')=="12">selected</#if>>城市规划</option>
	<option value="13" <#if xzfyBuwei.refer?default('0')=="13">selected</#if>>房屋拆迁</option>
	<option value="14" <#if xzfyBuwei.refer?default('0')=="14">selected</#if>>房屋登记</option>
	<option value="15" <#if xzfyBuwei.refer?default('0')=="15">selected</#if>>工商</option>
	<option value="16" <#if xzfyBuwei.refer?default('0')=="16">selected</#if>>质监</option>
	<option value="17" <#if xzfyBuwei.refer?default('0')=="17">selected</#if>>商务</option>
	<option value="18" <#if xzfyBuwei.refer?default('0')=="18">selected</#if>>物价</option>
	<option value="19" <#if xzfyBuwei.refer?default('0')=="19">selected</#if>>能源</option>
	<option value="20" <#if xzfyBuwei.refer?default('0')=="20">selected</#if>>交通</option>
	<option value="21" <#if xzfyBuwei.refer?default('0')=="21">selected</#if>>信息产业</option>
	<option value="22" <#if xzfyBuwei.refer?default('0')=="22">selected</#if>>邮政</option>
	<option value="23" <#if xzfyBuwei.refer?default('0')=="23">selected</#if>>烟草专卖</option>
	<option value="24" <#if xzfyBuwei.refer?default('0')=="24">selected</#if>>税务</option>
	<option value="25" <#if xzfyBuwei.refer?default('0')=="25">selected</#if>>人民银行</option>
	<option value="26" <#if xzfyBuwei.refer?default('0')=="26">selected</#if>>证监</option>
	<option value="27" <#if xzfyBuwei.refer?default('0')=="27">selected</#if>>保监</option>
	<option value="28" <#if xzfyBuwei.refer?default('0')=="28">selected</#if>>银监</option>
	<option value="29" <#if xzfyBuwei.refer?default('0')=="29">selected</#if>>外汇</option>
	<option value="30" <#if xzfyBuwei.refer?default('0')=="30">selected</#if>>财政</option>
	<option value="31" <#if xzfyBuwei.refer?default('0')=="31">selected</#if>>统计</option>
	<option value="32" <#if xzfyBuwei.refer?default('0')=="32">selected</#if>>审计</option>
	<option value="33" <#if xzfyBuwei.refer?default('0')=="33">selected</#if>>海关</option>
	<option value="34" <#if xzfyBuwei.refer?default('0')=="34">selected</#if>>商检</option>
	<option value="35" <#if xzfyBuwei.refer?default('0')=="35">selected</#if>>药监</option>
	<option value="36" <#if xzfyBuwei.refer?default('0')=="36">selected</#if>>卫生</option>
	<option value="37" <#if xzfyBuwei.refer?default('0')=="37">selected</#if>>计划生育</option>
	<option value="38" <#if xzfyBuwei.refer?default('0')=="38">selected</#if>>教育</option>
	<option value="39" <#if xzfyBuwei.refer?default('0')=="39">selected</#if>>文化</option>
	<option value="40" <#if xzfyBuwei.refer?default('0')=="40">selected</#if>>专利</option>
	<option value="41" <#if xzfyBuwei.refer?default('0')=="41">selected</#if>>商标</option>
	<option value="42" <#if xzfyBuwei.refer?default('0')=="42">selected</#if>>版权</option>	
	<option value="43" <#if xzfyBuwei.refer?default('0')=="43">selected</#if>>旅游</option>	
	<option value="44" <#if xzfyBuwei.refer?default('0')=="44">selected</#if>>其他</option>		
  </select>   </td>
 	</tr>
 	<tr <#if keywordList.size() gt 0>style="display:''"<#else>style="display:none"</#if>>      
				    ...
				    <td >
					  	 ...
					  	 <option value="${xzfyKeyword.keyword}" <#if xzfyKeyword.keyword==xzfyBuwei.keyWord>selected</#if>>
