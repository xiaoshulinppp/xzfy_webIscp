...
function savaToGetDoc(){
	...
	function getBuzhengtongzhi(){
		var url="../fzb_document_edit.jsp?action=new&d_filetype=doc&d_templateid=20131021111523&d_username=${Session['_USER_LOGIN_'].id}&caseId=${xzfyInfo.id}&d_type=补正通知";
		window.open(url,'mainwindow','location=no,resizable=yes');
	}
	...
	function getLiantongzhi(){
		var url="../fzb_document_edit.jsp?action=new&d_filetype=doc&d_templateid=20131021113134&d_username=panglei&caseId=${xzfyInfo.id}";
		window.open(url,'mainwindow','location=no,resizable=yes');
	}
	...
	function getJieantongzhi(){
		var url="../fzb_document_edit.jsp?action=new&d_filetype=doc&d_templateid=20131024142832&d_username=panglei&caseId=${xzfyInfo.id}";
		window.open(url,'mainwindow','location=no,resizable=yes');
	}
...
<input name="xzfyInfo.TApplicationDeptID" type="hidden" class="inputTextNormal" id="deptid" />
 <table  id="" class="ui-table ui-table-form "  ><!--表单填写项文字居右样式由 fn-text-right控制。td宽度在第一行标记 不要每行都标记-->
  ...
  <tr class="trcolor" id="handleCase" <#if xzfyInfo.status=='2'>style="display:none"</#if>>
	...
	<th>来件处理：</td>
	    <td class="tdFormInput">
<select name="scresult" 
<#if isShowBtn=='1' || xzfyInfo.status=='3' || xzfyInfo.status=='11'></#if> id="scresult" nbzcheck="y" altname="来件处理" onchange="handle_request()">
...
<table  id=""class="ui-table  "   width="50%">
  <tfoot>
      <tr>
            <td>
            <!--<input  class="ui-btn ui-btn-normal" type="button"  value="接待信息" onClick = "getFacePage('${xzfyInfo.id}');">
            -->
            <#if isShowBtn=='0'>
           		<input class="ui-btn ui-btn-normal" type="button"   value="保存" onClick="saveShencha()">
            	<!--
            	<input class="ui-btn ui-btn-normal" type="button"  value="提交" onClick="finishShencha()">
            	-->
            </#if>
...
function showTiaojie2(){
 ...
 var xmlHttp;
	...
	function getShouLi(){
		var url="../fzb_document_edit.jsp?action=new&d_filetype=doc&d_templateid=20131019091018&d_username=panglei&caseId=${xzfyId}";
		window.open(url,'mainwindow','location=no,resizable=yes');
	}
	
	function getRefuse(){
		var url="../fzb_document_edit.jsp?action=new&d_filetype=doc&d_templateid=20131019091855&d_username=panglei&caseId=${xzfyId}";
		window.open(url,'mainwindow','location=no,resizable=yes');	
	}
	
	function getYijian(){
		var url="../fzb_document_edit.jsp?action=new&d_filetype=doc&d_templateid=20131017113148&d_username=panglei&caseId=${xzfyId}";
		window.open(url,'mainwindow','location=no,resizable=yes');	
	}
	
	function getJianyi(){
		var url="../fzb_document_edit.jsp?action=new&d_filetype=doc&d_templateid=20131017134544&d_username=panglei&caseId=${xzfyId}";
		window.open(url,'mainwindow','location=no,resizable=yes');	
	}
