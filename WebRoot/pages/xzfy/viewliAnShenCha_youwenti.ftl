...
<tr></tr>
            ...
            <tr>
                ...
                <th >
					...
					<input type="radio"  <#if currentStatus gt 2 || xzfyInfo.check_status=='1'></#if> id="checkBox"
					onclick="checkBuzheng(1)" name="buzhengType" value="1" <#if xzfyInfo.isBuzheng=="1">checked</#if>>是
					<input type="radio" id="isFou" <#if currentStatus gt 2 || xzfyInfo.check_status=='1'></#if>
					onclick="checkBuzheng(0)" name="buzhengType" value="0" <#if xzfyInfo.isBuzheng=="0">checked</#if>>否
					<br/>
					<br/>
					<br/>
					<span id="bzContent"  style="display:none">
						<a id="bza" href="javascript:addContentType()"style="color:#930000">添加补正内容</a>
						<br/><br/><br/>
						<textarea id="bztext"></textarea>
					</span>
					<input type="hidden" name="bzCon" id="bzNeiRong" value=""></input>
					<input type="hidden"  id="receiveType" value="${xzfyInfo.receive_type}"></input>
				</td>
			...
			<th colspan=2>
				<span id="jieanfs2" <#if xzfyInfo.stadardResult?default('0')!='1'>style="display:none"</#if>>
					    ...
					    style="display:none" name="xzfyfb"</#if>>
						  ...
						  <option value="5" <#if xzfyInfo.finishBreakType?exists && (xzfyInfo.finishBreakType=="5" || xzfyInfo.finishBreakType=="2" ||xzfyInfo.finishBreakType=="3" )>selected</#if> >自愿撤回申请 </option>
			 ...
			 <#if currentStatus<12 && xzfyInfo.check_status=='0'>
				  ...
				  <tr align="center" id="jiean_info5" <#if status gt 3 || sdResult=="2" || sdResult=="3" || sdResult=="5"> style="display:''"<#else>style="display:none"</#if>>
						...
						<#if isView?has_content>
							...
							<input type="button"  style="border:1px solid;color:white;background:green;padding:3px;font-size:15px" value="结案" onClick="submitJiean()">
							</#if>
...
<table   id="dep" style="width:600px;">
	...
	<!-- 补正内容-->
	<div class="formDiv" id="bzTable" style="width:610px;position:absolute;display:block;">
...
function setKnowDate1(){
	...
	document.getElementById("usedate").value=knowDate;
 	...
 	function checkAnhao(){
 		...
 		$.ajax({
 			type:"post",
 			url:"../banan/checkAnhao.action?funcCode=${funcCode}&tempid=${xzfyInfo.id}",
 			datatype:"text",
 			data:"tempname="+anhao,
 			success:function(msg){
 				if(msg=='0')
 					document.getElementById("div1").innerHTML="<font color='red'>案号重复</font>";
 				else
 					document.getElementById("div1").innerHTML="<font color='green'>案号可用</font>";
 			},
 		});
 	}
 	function checkXwnum(){
 		var xwnum = $.trim($("#annum").val());
 		if(xwnum==""){
 			alert("文号不能为空");
 			$("#annum").focus();
 			return;
 		...
 		$.ajax({
 			type:"post",
 			url:"../banan/checkXwnum.action?funcCode=${funcCode}",
 			datatype:"text",
 			data:"tempname="+xwnum+"&tempid=${xzfyInfo.id}",
 			success:function(msg){
 				if(msg=='0')
 					document.getElementById("div2").innerHTML="<br/><br/><br/><a href='../banan/getcheckXwnumListByPage.action?funcCode=${funcCode}&tempid=${xzfyInfo.id}&tempname="+xwnum+"' target='_blank'><font color='red'>查看相同文号案件</font></a>";
 				else
 					document.getElementById("div2").innerHTML="<br/><br/><br/><font color='green'>无相同文号案件</font>";
 			},
 		})
 	}
 	function checkAppnum(obj){
 		var appnum = $.trim($(obj).val());
 		var div3 = $($(obj).next()).next();
 		$(div3).text("正在查重...");
 		$.ajaxSetup({cache:false});
 		//alert(appnum)
 		$.ajax({
 			type:"post",
 			url:"../banan/checkAppnum.action?funcCode=${funcCode}",
 			datatype:"text",
 			data:"tempname="+appnum+"&tempid=${xzfyInfo.id}",
 			success:function(msg){
 				//alert(msg)
 				//alert(div3)
 				if(msg=='0')
 					$(div3).html("<a href='../banan/getCheckAppnumListByPage.action?funcCode=${funcCode}&tempid=${xzfyInfo.id}&tempname="+appnum+"' target='_blank'><font color='red'>查看该申请人其他案件</font></a>");
 				else{
 					$(div3).html("<font color='green'>该申请人无其他案件</font>");
 				}
 			},
 		})
 	}
...
<script>
	$(function(){
		if($("#receiveType").val()=='4'){
			...
			if($cb[0].checked==true){
				$("#bzContent")[0].style.display="block";
			}
		}
		...
		br.click(function(){
			if(br[0].checked){
				document.getElementById("appDiv123").style.display="block";
			}
			else{
	...
	function addContentType(){
    	...
    	$("#bzTable").css( { position : 'absolute', 'top' : 750, 'left' : left + scrollLeft } ).show();   
	}
	function checkBuzheng(type){
		if(type==1){
			if($("#receiveType").val()=='4'){
				document.getElementById("bzContent").style.display="block";
			}
			...
			document.getElementById("bzContent").style.display="none";
