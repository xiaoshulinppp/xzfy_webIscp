...
<!--列表开始-->
 ...
 <div class="ui-list-nobtnbar fn-clear " id="bzTable">
    ...
    <table  id="" class="ui-table ui-table-form " >
     <input class="ui-btn ui-btn-normal" type="button" onclick="addContent()" value='完成'>
            <tr>
          <td  width="98%">
         <textarea class="textarea" style="width:90%;border:1px;border-style: solid;height:150px; "  id="bznr"> 该行政复议申请中有如下材料欠缺或表达不清，根据《中华人民共和国行政复议法实施条例》第29条的规定，请申请人于接待之日起5个工作日内补正。无正当理由逾期不补正的，视为放弃行政复议申请。补正申请材料所用时间不计入行政复议案件审理期限。
         </textarea>
          </td>
          </tr>  
...
（二十五）其他需要补正的情形。<input value="其他需要补正的情形。" type="checkbox" class="checkbox" onclick="addbz('（二十五）其他需要补正的情形。')">
	...
	document.getElementById('bznr').value=tObj.value;
