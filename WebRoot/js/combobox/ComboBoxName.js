...
Global_run_event_hookName = true;
Global_combo_arrayName    = new Array();
...
function ComboBoxName_choose(realval,txtval)
{
    this.value         = realval;
...
function ComboBoxName_mouseDown(e)
{
    var obj,len,el,i;
    ...
    if(elcl.indexOf("combo-")!=0)
    {
        ...
        len=Global_combo_arrayName.length
        for(i=0;i<len;i++)
        {
        
            curobj = Global_combo_arrayName[i]
...
function ComboBoxName_handleKey(e)
{
    ...
    if(elcl.indexOf("combo-")==0 && key!=13 && key!=40 && key!=38)
    {
        if(elcl.split("-")[1]=="input")
        {
            ...
            //else{ obj.view.style.zIndex = ++ComboBoxName.prototype.COMBOBOXZINDEX; obj.opslist.style.display='block';}
            
            obj.view.style.zIndex = ++ComboBoxName.prototype.COMBOBOXZINDEX;
   ...
   if (typeof(obj) == "object")
   {
		if (key == 40) // press the "↓" key  
		{
		    
		     {
					...
					//lineHilited = whichLineHiliteName(obj.opstable); 
...
function ComboBoxName_remove(index)
{
    this.options.remove(index)
}

function ComboBoxName_add()
{
    var i,arglen;
...
function ComboBoxName_build(arr)
{
    var str,arrlen
    arrlen=arr.length;
    ...
    for(var i=0;i<arrlen;i++)
    {
        strs[i] = '<tr>' +
			...
			'onMouseOver="on_MouseOverName(this);" onMouseOut="this.className=\'combo-item\'" >&nbsp;'+arr[i].text+'&nbsp;</td>' +
			'</tr>';
    }
...
function ComboBoxName_toggle() //when you click drop-down button runs after function ComboBox_mouseDown(e)
{
    ...
    if(this.opslist)
    {
        ...
        else
        {
            ...
            this.view.style.zIndex = ++ComboBoxName.prototype.COMBOBOXZINDEX
    ...
    else
    {
		...
		for(i=0;i<opln;i++)
		{
			 ...
			 if(astr==opart)
			 {
					...
					disableAllHiliteName(this.opstable);
...
function ComboBoxName()
{
    if(arguments.length==0)
    {
        self.status="ComboBoxName invalid - no name arg"
    }
    ...
    this.build  = ComboBoxName_build;
    this.make   = ComboBoxName_make;
    this.choose = ComboBoxName_choose;
    this.add    = ComboBoxName_add;
    this.toggle = ComboBoxName_toggle;
    this.update = ComboBoxName_update;
    this.remove = ComboBoxName_remove;
    this.make();
    ...
    Global_combo_arrayName[Global_combo_arrayName.length]=this;
    if(Global_run_event_hookName){ComboBoxName_init();}
}

ComboBoxName.prototype.COMBOBOXZINDEX = 1000 //change this if you must

function ComboBoxName_init() 
{
	if (document.addEventListener) {
		document.addEventListener("keyup", ComboBoxName_handleKey, false );
		document.addEventListener("mousedown", ComboBoxName_mouseDown, false );
	}
	else if (document.attachEvent) {
		document.attachEvent("onkeyup", function () { ComboBoxName_handleKey(window.event); } );
		document.attachEvent("onmousedown", function () { ComboBoxName_mouseDown(window.event); } );
	}
	
    Global_run_event_hookName = false;
}

function returnFalse(){return false}

function ComboBoxNameItem(text,value)
{
    this.text  = text;
    this.value = value;
}
...
function  whichLineHiliteName(myObj) //find out which line is hilited in the table. If return value =-1 means no row is hilited.
...
function disableAllHiliteName(myObj)
{
	var nTblLen;
	nTblLen = myObj.rows.length;
...
function on_MouseOverName(objThis)
{
...
function hideAllOpsListName()
{
    var len=Global_combo_arrayName.length;
    for(i=0;i<len;i++)
    {
...
function getAvailableCMBName()
{
	var curObj;
	var len=Global_combo_arrayName.length;
    for(i=0;i<len;i++)
    {
		if(Global_combo_arrayName[i].opslist)
		{
			if( Global_combo_arrayName[i].opslist.style.display != "none" )
			{
				curObj = Global_combo_arrayName[i];
				break;
			}
...
function setCmb_bfrSubmitName()
{
	...
	for(ii=1;ii<=nCmbCount;ii++)  //Loop for each comboBox
	{
		...
		//***********   Loop for all items in ComboBoxName     **********
		//
...
function restoreCmbName(arrCountryId,nCmbCount)
{
