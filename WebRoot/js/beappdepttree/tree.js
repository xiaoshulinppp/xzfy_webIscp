...
function TreeNode(){
	...
	this.init = function(name,tg,tab,parent,value,abbr){
		this.tg = tg;
		this.name = name;
		this.value = value;
		this.abbr = abbr;
	...
	this.addNode = function(name,value,abbr){
		node = this.tg.createTreeNode(name,this.tab+1,this,value,abbr);
...
function TreeGlobe(){
	...
	this.createTreeNode = function(name,tab,parent,value,abbr){
		node = new TreeNode();
		node.init(name,this,tab,parent,value,abbr);
		this.modifyNode(node,name,value,parent,abbr);
		return node;
	}
	...
	this.loadChildren = function(){
		...
		if (http_request.readyState == 4) {
			if (http_request.status == 200) {
				...
				items = receive.split("&^&");
				for(j=0;j<items.length-1;j+=4){
					nd = loadingNode.addNode(items[j+1],items[j+2],items[j+3]);
					nd.id = items[j];
				}
				if(items.length>1){
	...
	this.modifyNode = function(node,name,value,parent,abbr){
	}
}
