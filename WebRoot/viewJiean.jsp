					...
					try {
						...
						while (rs.next()) {
							...
							if(rs.getString("D_isqinggao").equals("未清稿")||rs.getString("D_isqinggao").equals("已清稿")){
								s_alert = "<script>window.location='fzb_document_dafutongzhi_edit.jsp?action=yesrevision&caseId="+ms_cassid+"&d_username="+ms_userrName+"&d_recordid="+rs.getString("D_RecordID")+"&mstype="+ms_type+"&userrid="+userrid+"'</script>";
								closeAll(con, sttmt, rs);
							response.getWriter().write(s_alert);
							}
						}
