...
<%
	...
	con = getConn();
	ms_all=ms_caseId+";"+ms_finishtype;
	...
	if (ms_Action.equals("new")) {
		...
		response.sendRedirect("fzb_default_dafutongzhijiean_list.jsp?caseId="+ms_caseId+"&d_username"+ms_Author);
	}
	...
	ms_WebUrl += "eWebOffice/eWebOfficeAction4.jsp";
%>
		...
		<div id="bdy">
			...
			<hr />
						<%if(ms_NodeDesc.equals("阅读")){ %>
							...
							<% }else{%>
			<form id="form1" name="form1" method="post" action="fzb_document_dafutongzhijiean_save.jsp?caseId=<%=ms_caseId%>&d_username=<%=ms_Author%>&mstype=<%=ms_type%>&userrid=<%=userrid%>"
				...
				<table class="">
							...
							<!--  <input type="button" class="btn4"
								onclick="location.href='fzb_default_dafutongzhijiean_list.jsp?caseId=<%=ms_caseId%>&d_username=<%=ms_Author%>'" value="查看历史版本">-->
							...
							<!--  <input type="button" class="btn4"
								onclick="location.href='fzb_default_dafutongzhijiean_list.jsp?caseId=<%=ms_caseId%>&d_username=<%=ms_Author%>'" value="返回列表">*/-->
							<br>
							<br>
...
function acceptAllRevisions2(){
	...
	function createApp(){
		...
		var options = {
			...
			url:'<%="fzb_document_dafutongzhijiean_save.jsp?caseId="+ms_caseId+"&d_username="+ms_Author+"&mstype="+ms_type+"&userrid="+userrid%>', //提交给哪个执行 
			type:'POST',
			success: callbackfun
		};
