...
<table width="98%" border="0" align="center" cellpadding="3" cellspacing="0" class="tabout" id="color_table">
   ...
   <tr class="trcolor">
    ...
    ${r"<#else>"}
     ...
     ${r"</#if>"}
    ${r"<#if"} field?has_content>${r"${field}"}${r"</#if>"}</td>
    ${r"</#if>"}
    ${r"<#assign seq = seq +1>"}
    ${r"</#list>"}
    ${r"<#list displayInfo.operationList as op>"}
       ${r"<#if op.single!=0 >"}
       <td class="tdlistCenter"><a href="../infogather/${r"${op.abbr}"}${className}.action?opName=${r"${op.abbr}"}&entityId=${r"${keyColumn}"}&funcCode=${r"${funcCode}"}&pageNum=${r"${pageNum}"}">${r"${op.name}"}</a></td>
       ${r"</#if>"}
    ${r"</#list>"}
  </tr>
  ...
  <tr>
    <td><table  border="0" align="right" cellpadding="3" cellspacing="2">
      <tr align="center">
        ...
        <td><a href="#" onclick="gotoPage(${r"'${totalPage}'"},${r"'${funcCode}'"})"><img src="../${r"${Session['_USER_STYLE_'].imagePath}"}/button_go.gif" alt="${r"${action.getText('return.text')}"}" width="20" height="16" border="0"></a></td>
      </tr>
    </table></td>
  </tr>
</table>
