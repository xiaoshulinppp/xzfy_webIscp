  ...
  <tr>
    <td><table  border="0" align="right" cellpadding="3" cellspacing="2">
      <tr align="center">
        ...
        <td align="center"><a href ="${className}QueryList.action?pageNum=1&funcCode=${r"${funcCode}"}&operationCode=${gatherabbr}&${keyvar}=${r"${"}${keyvar}${r"}"}">${r"${action.getText('firstpage')}"}</a></td>
         ${r"<#assign upPage = pageNum -1>"}
       ${r" <#if upPage <1 >"}
          ${r"<#assign upPage = 1>"}
        ${r"</#if>"}       
        <td align="center"><a href ="${className}QueryList.action?pageNum=${r"${upPage}"}&funcCode=${r"${funcCode}"}&operationCode=${gatherabbr}&${keyvar}=${r"${"}${keyvar}${r"}"}">${r"${action.getText('uptopage')}"}</a></td>
        ${r"<#assign downPage = pageNum +1>"}
	
        ${r"<#if (downPage > totalPage)>"}
          ${r"<#assign downPage = totalPage >"}
        ${r"</#if>"}
        <td align="center"><a href ="${className}QueryList.action?pageNum=${r"${downPage}"}&funcCode=${r"${funcCode}"}&operationCode=${gatherabbr}&${keyvar}=${r"${"}${keyvar}${r"}"}">${r"${action.getText('downtopage')}"}</a></td>
        <td align="center"><a href ="${className}QueryList.action?pageNum =${r"${totalPage}"}&funcCode=${r"${funcCode}"}&operationCode=${gatherabbr}&${keyvar}=${r"${"}${keyvar}${r"}"}">${r"${action.getText('endpage')}"}</a></td>
        ...
        <td><a href="#" onclick="gotoPage('${r"${totalPage}"}','${r"${funcCode}"}','${r"${id}"}')"><img src="../${r"${Session['_USER_STYLE_'].imagePath}"}/button_go.gif" alt="${r"${action.getText('goto.text')}"}" width="20" height="16" border="0"></a></td>
      </tr>
    </table></td>
  </tr>
</table>
<script>
  ...
  function gotoPage(pMax,funcCode,infoId) {
    ...
    window.location="${className}QueryList.action?pageNum="+window.opListForm.gotoPageNum.value+"&funcCode="+funcCode+"&${keyvar}="+infoId+"&operationCode=${gatherabbr}";
  }
