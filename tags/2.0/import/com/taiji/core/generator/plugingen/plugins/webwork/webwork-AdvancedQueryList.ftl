...
<!--javascript脚本添加位置结束-->
<body>
<form name="exsearchListForm" method="post">
<!--您的位置表格开始-->
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="yourPlace" id=control >
  <tr>
        ...
        <td>${r"${action.getText('current.location')}"}--${plugin.functionName}管理列表</td>
      </tr>
</table>
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" class="tabtitle" id=control>
  <tr>
    <td>${plugin.functionName}查询列表</td>
  </tr>
</table>
...
<table width="98%" border="0" align="center" cellpadding="3" cellspacing="0" class="tabout" id="color_table">
   ...
   <tr class="trcolor">
    ...
    ${r"</#list>"}
    ${r"<#list displayInfo.operationList as op>"}
          <td class="tdlistCenter"><a href="../info/view${className}AdvancedQuery.action?opName=view&entityId=${r"${keyColumn}"}&funcCode=${r"${funcCode}"}&pageNum=${r"${pageNum}"}">查看</a></td>
  ...
  <tr>
    <td><table  border="0" align="right" cellpadding="3" cellspacing="2">
      <tr align="center">
        ...
        <td align="center"><a href ="../info/${className}AdvancedQuery.action?pageNum=1&funcCode=${r"${funcCode}"}">${r"${action.getText('firstpage')}"}</a></td>
         ${r"<#assign upPage = pageNum -1>"}
        ${r"<#if upPage <1 >"}
          ${r"<#assign upPage = 1>"}
        ${r"</#if>"}       
        <td align="center"><a href ="../info/${className}AdvancedQuery.action?pageNum=${r"${upPage}"}&funcCode=${r"${funcCode}"}">${r"${action.getText('uptopage')}"}</a></td>
        ${r"<#assign downPage = pageNum +1>"}
	
        ${r"<#if (downPage > totalPage)>"}
          ${r"<#assign downPage = totalPage >"}
        ${r"</#if>"}
        <td align="center"><a href ="../info/${className}AdvancedQuery.action?pageNum=${r"${downPage}"}&funcCode=${r"${funcCode}"}">${r"${action.getText('downtopage')}"}</a></td>
        <td align="center"><a href ="../info/${className}AdvancedQuery.action?pageNum =${r"${totalPage}"}&funcCode=${r"${funcCode}"}">${r"${action.getText('endpage')}"}</a></td>
...
<script>
  function gotoPage(pMax,funcCode) {
    ...
    window.location="../info/${className}AdvancedQuery.action?pageNum="+window.${objectName}ListForm.gotoPageNum.value+"&funcCode="+funcCode;
  }
      function returnPage() {
   
     document.location.href="../info/${className}AdvancedQueryTerm.action?funcCode=${r"${"}funcCode${r"}"}";
    }
</script>
