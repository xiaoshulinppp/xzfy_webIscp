...
<table width="98%" border="0" align="center" cellpadding="3" cellspacing="0" class="tabout" id="color_table">
   ...
   <tr class="trcolor">
    ...
    ${r"<#list"} displayInfo.operationList as op>
       ...
       ${r"</#if>"}
       ${r"<#if"} op.abbr!='submit'>
       <a href="javascript:FormClick('${r"${op.abbr}"}','${r"${keyColumn}"}','${r"${funcCode}"}','${r"${"}${relationObjectName}Id${r"}"}','${r"${pageNum}"}')">${r"${op.name}"}</a></td>
       ${r"</#if>"}
       ${r"</#if>"}
    ${r"</#list>"}
    ${r"</#if>"}
    ...
    ${r"<#list"} displayInfo.operationList as op>
       ${r"<#if"} op.single >
       <td class="tdlistCenter"><a href="javascript:FormClick('${r"${op.abbr}"}','${r"${keyColumn}"}','${r"${funcCode}"}','${r"${"}${relationObjectName}Id${r"}"}','${r"${pageNum}"}')">${r"<#if"} op.abbr=="check">修改${r"</#if>"}${r"<#if"} op.abbr!="check">${r"${op.name}"}${r"</#if>"}</a></td>
       ${r"</#if>"}
    ${r"</#list>"}
    ${r"</#if>"}
    ${r"<#if"} opName=='view'||opName=='submit'>
       <td class="tdlistCenter">
       <a href="javascript:FormClick('view','${r"${keyColumn}"}','${r"${funcCode}"}','${r"${"}${relationObjectName}Id${r"}"}','${r"${pageNum}"}')">查看</a></td>
  ...
  <tr>
    <td><table  border="0" align="right" cellpadding="3" cellspacing="2">
      <tr align="center">
        ...
        <td align="center"><a href ="../infogather/list${className}InLine.action?pageNum=1&funcCode=${r"${funcCode}"}&opName=${r"${opName}"}&${relationObjectName}Id=${r"${"}${relationObjectName}Id${r"}"}">${r"${action.getText('firstpage')}"}</a></td>
         ${r"<#assign"} upPage = pageNum -1>
        ${r"<#if"} upPage <1 >
          ${r"<#assign"} upPage = 1>
        ${r"</#if>"}       
        <td align="center"><a href ="../infogather/list${className}InLine.action?pageNum=${r"${upPage}"}&funcCode=${r"${funcCode}"}&opName=${r"${opName}"}&${relationObjectName}Id=${r"${"}${relationObjectName}Id${r"}"}">${r"${action.getText('uptopage')}"}</a></td>
        ${r"<#assign"} downPage = pageNum +1>
	
        ${r"<#if"} (downPage > totalPage)>
          ${r"<#assign"} downPage = totalPage >
        ${r"</#if>"}
        <td align="center"><a href ="../infogather/list${className}InLine.action?pageNum=${r"${downPage}"}&funcCode=${r"${funcCode}"}&opName=${r"${opName}"}&${relationObjectName}Id=${r"${"}${relationObjectName}Id${r"}"}">${r"${action.getText('downtopage')}"}</a></td>
        <td align="center"><a href ="../infogather/list${className}InLine.action?pageNum =${r"${totalPage}"}&funcCode=${r"${funcCode}"}&opName=${r"${opName}"}&${relationObjectName}Id=${r"${"}${relationObjectName}Id${r"}"}">${r"${action.getText('endpage')}"}</a></td>
        ...
        <td><a href="#" onclick="gotoPage('${r"${totalPage}"}','${r"${funcCode}"}','${r"${opName}"}','${r"${"}${relationObjectName}Id${r"}"}')"><img src="../${r"${Session['_USER_STYLE_'].imagePath}"}/button_go.gif" alt="${r"${action.getText('goto.text')}"}" width="20" height="16" border="0"></a></td>
      </tr>
    </table></td>
  </tr>
</table>
<script>
 ...
 if(opName == 'delete') {
     ...
     window.open("../infogather/"+opName+"${className}InLine.action?opName="+opName+"&funcCode="+funcCode,'popwin','scrollbars=yes,resizable=yes,width=800,height=600')
     }else{
    window.open("../infogather/"+opName+"${className}InLine.action?opName="+opName+"&funcCode="+funcCode+"&${relationObjectName}Id="+info1Id,'popwin','scrollbars=yes,resizable=yes,width=800,height=600')
     }
  }
  function FormClick(opName,${objectName}Id,funcCode,info1Id,pageNum){
  window.open("../infogather/"+opName+"${className}InLine.action?opName="+opName+"&funcCode="+funcCode+"&entityId="+${objectName}Id+"&${relationObjectName}Id="+info1Id+"&pageNum="+pageNum,'popwin','scrollbars=yes,resizable=yes,width=800,height=600')
     }
  ...
  function gotoPage(pMax,funcCode,opName,info1Id) {
    ...
    window.location="../infogather/list${className}InLine.action?pageNum="+window.${objectName}ListForm.gotoPageNum.value+"&funcCode="+funcCode+"&opName="+opName+"&${relationObjectName}Id="+info1Id;
  }
</script>
