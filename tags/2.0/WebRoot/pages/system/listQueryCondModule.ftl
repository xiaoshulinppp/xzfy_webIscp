...
<body>
<form name="queryListForm" method="post">
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="yourPlace">
  <tr>
         <td>${action.getText("current.location")}:${action.getText("querylist.title")}</td>
      </tr>
</table>
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" class="tabtitle">
  <tr>
    <td>${action.getText("querylist.title")}</td>
  </tr>
</table>
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" class="tabMainTop">
  <tr>
    <td><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
      <tr>
          <td align="right">
          <input  name="Button" type="button" class="button" value="复杂查询" onclick="complexQuery()">
          <#list displayInfo.operationList as op>
            <#if op.single!=1>
              <#if op.abbr="create">
              <input id ="createId" name="Button" type="button" class="button" value="新查询" onclick="FormSubmit('create')">
              <#else>
...
<table width="98%" border="0" align="center" cellpadding="3" cellspacing="0" class="tabout" >
   ...
   <tr class="trcolor">
    ...
    <#list displayInfo.operationList as op>
       <#if op.single!=0 >
       <td class="tdlistCenter"><a href="../system/${op.abbr}AdvanceQueryTerm.action?opName=${op.abbr}&entityId=${keyColumn}&funcCode=${funcCode}&pageNum=${pageNum}"><img src="../${Session["_USER_STYLE_"].imagePath}/sicons/${op.image}" alt="${op.name}" width="16" height="16" border="0"></a></td>
       </#if>
    </#list>
  </tr>
  </#list>
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" class="tabMainBot">
  <tr>
    <td>
        ...
        <a href ="../system/listQuery.action?pageNum=1&funcCode=${funcCode}">${action.getText("firstpage")}</a>
         <#assign upPage = pageNum -1>
        <#if upPage <1 >
          <#assign upPage = 1>
        </#if>       
        <a href ="../system/listQuery.action?pageNum=${upPage}&funcCode=${funcCode}">${action.getText("uptopage")}</a>
        <#assign downPage = pageNum +1>
	
        <#if (downPage > totalPage)>
          <#assign downPage = totalPage >
        </#if>
        <a href ="../system/listQuery.action?pageNum=${downPage}&funcCode=${funcCode}">${action.getText("downtopage")}</a>
        <a href ="../system/listQuery.action?pageNum =${totalPage}&funcCode=${funcCode}">${action.getText("endpage")}</a>
        ${action.getText("di")}
...
<script>
  function FormSubmit(opName) {
     submitDisableButton();
     window.queryListForm.action="../system/advanceQueryTerm.action?opName="+opName;
     window.queryListForm.submit();
  }
  ...
  function gotoPage(pMax) {
    ...
    var pLastNum = window.queryListForm.gotoPageNum.value ;
    ...
    window.location="../system/listQuery.action?pageNum="+pLastNum+"&funcCode=${funcCode}";
  }
  function complexQuery() {
  window.queryListForm.action="../system/complextQueryTerm.action?opName=create";
  window.queryListForm.submit();
  }
</script>
