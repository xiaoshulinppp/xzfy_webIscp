...
<html xmlns="http://www.w3.org/1999/xhtml">
	       ...
	       <thead class=""  id="" name="" >
			...
			<#list caseList as xzfyInfo>
				<tr >
					...
					<td  >
						<#if xzfyInfo.status?has_content && xzfyInfo.status=="4">
							案件办理
						<#elseif xzfyInfo.status?has_content && xzfyInfo.status=="5">
							延期审批
						<#elseif xzfyInfo.status?has_content && xzfyInfo.status=="6">
							案件延期
						<#elseif xzfyInfo.status?has_content && xzfyInfo.status=="7">
							中止审批
						<#elseif xzfyInfo.status?has_content && xzfyInfo.status=="8">
							案件中止
						<#elseif xzfyInfo.status?has_content && xzfyInfo.status=="9">
							恢复审理审批
						<#elseif xzfyInfo.status?has_content && xzfyInfo.status=="10">
							恢复审理
						<#elseif xzfyInfo.status?has_content && xzfyInfo.status=="11">
							结案审批
						<#elseif xzfyInfo.status?has_content && xzfyInfo.status=="12">
							结案
						</#if>
					</td>
					<td  id="cxs" style="display:block" >
						<a href="../banan/banan.action?funcCode=${funcCode}&tempid=${xzfyInfo.id}">办理</a>&nbsp;		
					</td>
				</tr>
			</#list> 
	     	</tbody>
	...
	<div class="ui-page">
	    <span class="ui-page-item ui-page-item-info"> 第${pNum}/${totalPage}页   共${caseAllList.size()}条</span>
	    ...
	    <a href="../banan/showanjianbanliList.action?pNum=${totalPage}&funcCode=${funcCode}&listkeyword=${listkeyword}" class="ui-page-item ui-page-last">尾页</a></span>	    
	</div> 
