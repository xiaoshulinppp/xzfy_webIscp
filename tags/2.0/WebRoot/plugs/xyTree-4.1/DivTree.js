...
xyTree.TreeConfig = {
	/*
	 * 以下5个分别是关闭的根节点，打开的根节点，关闭的文件夹， 打开的文件夹，叶节点
	 */
	...
	buzhongIcon : '../plugs/xyTree-4.1/images/buzhong.gif',
	quanzhongIcon : '../plugs/xyTree-4.1/images/quanzhong.gif',
	jubuzhongIcon : '../plugs/xyTree-4.1/images/jubuzhong.gif',
...
xyTree.CONSTANT = {
	...
	// 新加一个DOM对象ID：2011-04-29 Kerbol
	treeFlodImgID : 'imgfold',
...
xyTree.fun = {
	slowChange : function(treeXuhao, nodeXuhao, timenum) {
		...
		if (arguments.length == 2) // 如果用户未定义时间
			setTimeout("xyTree.fun.slowChange(" + treeXuhao + "," + nodeXuhao
					+ ")", xyTree.CONSTANT.timenum);
		else
			// 如果用户定义了时间
			setTimeout("xyTree.fun.slowChange(" + treeXuhao + "," + nodeXuhao
					+ "," + timenum + ")", timenum);
 ...
 * 给树选中所有的复选框，不影响形状
 * 
 * date 2011-05-04 author Kerbol
 */
xyTree.DivTree.prototype.initCheckAllCheckBox = function() {
	var root = this.tree.root;
	root.getHtmlElementfuxuanimg().onclick();
}
...
xyTree.DivTree.prototype.creatediv = function() {
	...
	if (!this.img) { // 如果用户没有指定图标
		...
		img.onclick = function() {
			var divbody = this.parentNode.parentNode.lastChild;
			divbody.style.display = (divbody.style.display == 'block') ? 'none'
					: 'block';
		}
	}
 ...
 * date 2011-05-05
 */
xyTree.DivTree.prototype.getNodesAlls = function() {
	function getNodesdiguiAlls(arr, node) {
		function getNodesdiguiAllSelecteds(node2) { // 闭包
			arr.push(node2);
			for ( var i = 0; i < node2.child.length; i++)
				getNodesdiguiAllSelecteds(node2.child[i]);
		}
		if (node.checked == 0)
			/* 空函数体 */;
		else if (node.checked == 1) {
			arr.push(node);
			for ( var i = 0; i < node.child.length; i++)
				getNodesdiguiAllSelecteds(node.child[i]);
		} else {
			arr.push(node);
			for ( var i = 0; i < node.child.length; i++)
				getNodesdiguiAlls(arr, node.child[i]);
		}
	}
	var resultArr = [];
	getNodesdiguiAlls(resultArr, this.tree.root);
	return resultArr;
}
